...
func (ExternalSecretsAnalyzer) externalSecret(a common.Analyzer) ([]common.Result, error) {
	var mutex = &sync.Mutex{}

	kind := "ExternalSecret"
	result := &v1beta1.ExternalSecretList{}
	...
	for _, es := range result.Items {
		...
		for _, condition := range es.Status.Conditions {
			if condition.Status == v1.ConditionFalse {
				...
				evt, err := fetchLatestEvent(a.Context, k8sclient, es.Namespace, es.Name)
				if err != nil || evt == nil {
					continue
				}
				...
				if len(failures) > 0 {
					preAnalysis[fmt.Sprintf("%s/%s", es.Namespace,
						es.Name)] = common.PreAnalysis{
						ExternalSecret: es,
			...
			for key, value := range preAnalysis {
				...
				parent, _ := util.GetParent(a.Client, value.ExternalSecret.ObjectMeta)
